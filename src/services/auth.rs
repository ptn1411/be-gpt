...
impl AuthService {
    ...
    pub async fn register(
        ...
        let (token, expires_at, session_id) = Self::generate_token(&user, jwt_secret, 1)?; // 1 hour for now
    ...
    pub async fn login(
        ...
        let (token, expires_at, session_id) = Self::generate_token(&user, jwt_secret, 1)?; // 1 hour
    ...
    pub async fn refresh_token(
        ...
        let (new_token, new_expires_at, _) = Self::generate_token(&user, jwt_secret, 1)?;
    ...
    pub async fn get_session(db: &Database, user_id: Uuid, token: &str) -> AppResult<UserSession> {
        let session: Session = sqlx::query_as(
    ...
    fn parse_ip(raw: Option<&str>) -> Option<String> {
