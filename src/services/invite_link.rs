...
pub async fn create_invite_link(
    ...
    if let Some(chat_id) = request.chat_id {
        let row =
            sqlx::query("SELECT role FROM chat_participants WHERE chat_id = $1 AND user_id = $2")
                .bind(chat_id)
                .bind(user_id)
                .fetch_optional(pool)
                .await?;
        ...
        match row {
            Some(r) => {
                ...
                if role != Some("admin".to_string()) && role != Some("owner".to_string()) {
...
pub async fn use_invite_link(
    ...
    let already_used =
        sqlx::query("SELECT id FROM invite_link_uses WHERE invite_link_id = $1 AND user_id = $2")
            .bind(invite_link.id)
            .bind(user_id)
            .fetch_optional(&mut *tx)
            .await?;
    ...
    let chat_id = match invite_link.link_type.as_str() {
        "group" => {
            let chat_id = invite_link.chat_id.ok_or_else(|| {
                AppError::Internal(anyhow::Error::msg("Group link missing chat_id"))
            })?;

            // Check if user is already a participant
            let existing =
                sqlx::query("SELECT id FROM chat_participants WHERE chat_id = $1 AND user_id = $2")
                    .bind(chat_id)
                    .bind(user_id)
                    .fetch_optional(&mut *tx)
                    .await?;
    ...
    sqlx::query(
        "UPDATE invite_links SET current_uses = current_uses + 1, updated_at = NOW() WHERE id = $1",
    )
    .bind(invite_link.id)
    .execute(&mut *tx)
    .await?;
