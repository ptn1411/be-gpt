...
impl BotService {
    ...
    pub async fn process_bot_message(
        ...
        let message =
            Self::create_bot_message(db, chat_id, bot_id, &response_text, inline_keyboard).await?;
    ...
    pub async fn handle_callback(
        ...
        if original.sender_id != bot_id {
            return Err(AppError::BadRequest(
                "Invalid callback: message is not from this bot".to_string(),
            ));
        }
    ...
    fn generate_notification_toggle_response(
        enabled: bool,
    ) -> (String, Option<Vec<Vec<InlineButton>>>) {
        ...
        let toggle_callback = if enabled {
            "notifications_off"
        } else {
            "notifications_on"
        };
