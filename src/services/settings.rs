...
const SUPPORTED_ACCENT_COLORS: &[&str] = &[
    "#0284c7", "#0d9488", "#16a34a", "#f97316", "#a855f7", "#ec4899", "#facc15", "#64748b",
];
...
impl SettingsService {
    ...
    pub async fn clear_cache(db: &Database, user_id: Uuid) -> AppResult<DataStorageSettings> {
        ...
        if tokio::fs::metadata(&cache_dir).await.is_ok() {
            tokio::fs::remove_dir_all(&cache_dir)
                .await
                .map_err(|e| AppError::Internal(anyhow::anyhow!("Failed to clear cache: {}", e)))?;
    ...
    pub async fn update_appearance(
        ...
        let accent_color = match accent_color {
            Some(color) => Some(Self::normalize_accent_color(&color)?),
            None => None,
        };
    ...
    pub async fn terminate_device(
        ...
        let session: Session =
            sqlx::query_as("SELECT * FROM sessions WHERE id = $1 AND user_id = $2")
                .bind(device_id)
                .bind(user_id)
    ...
    fn normalize_accent_color(color: &str) -> AppResult<String> {
        if color.len() != 7 || !color.starts_with('#') {
            return Err(AppError::InvalidAccentColor(
                "Accent color must be in the format #RRGGBB".to_string(),
            ));
        }
        ...
        {
            return Err(AppError::InvalidAccentColor(
                "Accent color must be a hex value (0-9, A-F)".to_string(),
            ));
        }

        let normalized = color.to_ascii_lowercase();

        if !SUPPORTED_ACCENT_COLORS.contains(&normalized.as_str()) {
            return Err(AppError::InvalidAccentColor(format!(
                "Accent color must be one of: {}",
                SUPPORTED_ACCENT_COLORS.join(", ")
            )));
        }
