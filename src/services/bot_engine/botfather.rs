...
impl BotFather {
    ...
    pub async fn handle_message(
        ...
        match session.state {
            ConversationState::AwaitingBotName => Self::handle_bot_name_input(user_id, text).await,
    ...
    pub async fn handle_command(
        ...
        if cmd.command == "cancel" {
            Self::clear_session(user_id);
            return Ok(Some(BotFatherResponse::success(
    ...
    async fn cmd_mybots(db: &Database, user_id: Uuid) -> AppResult<BotFatherResponse> {
        ...
        for bot in bots {
            ...
            text.push_str(&format!(
                "{} {} ({})\n   ID: {}\n\n",
                status, bot.name, username, bot.id,
            ));
        }
    ...
    async fn cmd_deletebot(
        ...
        match BotEngineService::delete_bot(db, bot_id, user_id).await {
            ...
            Err(AppError::BotNotFound) => Ok(BotFatherResponse::error("❌ Bot not found.")),
    ...
    async fn cmd_setwebhook(
        ...
        match BotEngineService::set_webhook(db, bot_id, url).await {
            ...
            Err(AppError::InvalidWebhookUrl) => Ok(BotFatherResponse::error(
                "❌ Invalid webhook URL. Must be a valid HTTPS URL.",
            )),
            Err(AppError::WebhookError(msg)) => Ok(BotFatherResponse::error(format!(
    ...
    async fn cmd_clearwebhook(
        ...
        let bot_id_str = match cmd.first_arg() {
            Some(id) => id,
            None => {
                return Ok(BotFatherResponse::error("❌ Usage: /clearwebhook <bot_id>"));
            }
        };
    ...
    async fn cmd_token(
        ...
        let regenerate =
            cmd.args.get(1).map(|s| s.to_lowercase()) == Some("regenerate".to_string());
    ...
    async fn cmd_removebot(
        ...
        let bot_id_str = match cmd.first_arg() {
            Some(id) => id,
            None => {
                return Ok(BotFatherResponse::error("❌ Usage: /removebot <bot_id>"));
            }
        };
...
mod tests {
    ...
    fn test_is_botfather_command() {
        ...
        assert!(BotFather::is_botfather_command(
            &ParsedCommand::parse("/cancel").unwrap()
        ));
        assert!(!BotFather::is_botfather_command(
