...
impl WebSocketService {
    ...
    pub async fn broadcast_typing(
        ...
        ws_manager
            .broadcast_to_room(chat_id, event, Some(user_id))
            .await;
    }
    ...
    /// Broadcast participant role updates to a chat
    pub async fn broadcast_participant_role_updated(
        ws_manager: &Arc<WsManager>,
        chat_id: Uuid,
        user_id: Uuid,
        role: String,
        ...
        let event = ServerEvent::ParticipantRoleUpdated {
            chat_id,
            user_id,
            role,
            updated_by,
        ...
        ws_manager
            .broadcast_to_chat_participants(participant_ids, event, None)
            .await;
    }

    /// Broadcast participant removal to the remaining chat members
    ...
    pub async fn send_call_declined(ws_manager: &Arc<WsManager>, call_id: Uuid, caller_id: Uuid) {
    ...
    pub async fn send_user_busy(ws_manager: &Arc<WsManager>, call_id: Uuid, caller_id: Uuid) {
