...
mod integration_tests {
    ...
    async fn test_appearance_settings_update() {
        ...
        assert_eq!(updated_appearance.accent_color, "#0d9488");
    ...
    #[tokio::test]
    async fn test_invalid_accent_color_rejected() {
        let db = setup_test_db().await;
        let user = create_test_user(&db).await;

        let result = SettingsService::update_appearance(
            &db,
            user.id,
            Some("light".to_string()),
            Some("#12345Z".to_string()),
            None,
            None,
            None,
            None,
        ...
        assert!(matches!(result, Err(AppError::InvalidAccentColor(_))));
    ...
    async fn test_current_session_protection() {
        ...
        let result =
            SettingsService::terminate_device(&db, user.id, session.id, &session.token).await;
    ...
    async fn test_terminate_nonexistent_device() {
        ...
        let result =
            SettingsService::terminate_device(&db, user.id, fake_device_id, &session.token).await;
