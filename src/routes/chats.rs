...
use crate::{
    error::AppResult,
    models::{
        BotPublicResponse, ChatDetailResponse, ChatResponse, MessageResponse, ParticipantResponse,
    },
...
pub fn routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            ...
            get(get_chat)
                .delete(delete_chat)
                .patch(update_group_metadata),
        )
        .route(
            "/:chat_id/participants",
            get(list_chat_participants).post(add_chat_participant),
        )
        .route(
            "/:chat_id/participants/:user_id",
            axum::routing::delete(remove_chat_participant),
        )
        .route(
            "/:chat_id/participants/:user_id/role",
...
#[derive(Debug, Serialize)]
pub struct ParticipantResponseWrapper {
...
async fn list_chat_participants(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
    Path(chat_id): Path<Uuid>,
    ...
    let participants = ChatService::list_chat_participants(&state.db, chat_id, user_id).await?;
...
) -> AppResult<Json<ParticipantResponseWrapper>> {
    let actor_id = get_current_user_id(&state, &headers).await?;

    let participant =
        ChatService::add_chat_participant(&state.db, chat_id, actor_id, req.user_id).await?;

    Ok(Json(ParticipantResponseWrapper { participant }))
}

async fn remove_chat_participant(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
    ...
    ChatService::remove_chat_participant(&state.db, chat_id, actor_id, user_id).await?;

    let participant_ids = ChatService::get_participant_ids(&state.db, chat_id).await?;
    WebSocketService::broadcast_participant_removed(
        &state.ws_manager,
        chat_id,
        user_id,
        actor_id,
    ...
    Ok(Json(SimpleMessage {
        message: "Participant removed successfully".to_string(),
    }))
}
...
async fn update_participant_role(
    ...
    let participant = ChatService::update_chat_participant_role(
        &state.db,
        chat_id,
        actor_id,
        user_id,
    ...
    let participant_ids = ChatService::get_participant_ids(&state.db, chat_id).await?;
    WebSocketService::broadcast_participant_role_updated(
        &state.ws_manager,
        chat_id,
        participant.id,
...
async fn update_group_metadata(
    ...
    let actor_id = get_current_user_id(&state, &headers).await?;
    let UpdateGroupRequest { name, avatar } = req;

    let chat =
        ChatService::update_group_chat_metadata(&state.db, chat_id, actor_id, name, avatar).await?;
...
async fn create_group(
    ...
    let admins = req.admins.unwrap_or_default();
    let chat =
        ChatService::create_group(&state.db, user_id, &req.name, req.participant_ids, admins)
            .await?;
