...
use crate::{
    error::AppResult, models::UserPublic, routes::auth::get_current_user_id, services::UserService,
    AppState,
};
...
async fn get_users(
    ...
    let users = if query.connected_only.unwrap_or(true) {
        ...
        UserService::get_connected_users(&state.db, current_user_id, query.search.as_deref(), limit)
            .await?
