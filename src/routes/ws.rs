...
use crate::{
    ...
    services::{ChatService, UserService, WebSocketService},
    AppState,
};
...
async fn start_typing(
    ...
    WebSocketService::broadcast_typing(&state.ws_manager, chat_id, user_id, user.name, true).await;
...
async fn stop_typing(
    ...
    // Broadcast typing event
    WebSocketService::broadcast_typing(&state.ws_manager, chat_id, user_id, user.name, false).await;
