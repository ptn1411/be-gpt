...
const ALLOWED_EXTENSIONS: &[&str] = &[
    // Images
    "jpg", "jpeg", "png", "gif", "webp", "bmp", "svg", // Documents
    "pdf", "doc", "docx", "xls", "xlsx", "ppt", "pptx", "txt", "csv", // Archives
    "zip", "rar", "7z", "tar", "gz", // Audio
    "mp3", "wav", "ogg", "m4a", // Video
...
const MAGIC_BYTES: &[MagicBytes] = &[
    ...
    MagicBytes {
        bytes: &[0x00, 0x00, 0x00, 0x14, 0x66, 0x74, 0x79, 0x70],
        extensions: &["mp4"],
        mime_type: "video/mp4",
    },
    MagicBytes {
...
async fn upload_file(
    ...
    while let Some(field) = multipart
        .next_field()
        .await
        .map_err(|e| AppError::Internal(anyhow::anyhow!("Failed to read multipart: {}", e)))?
    {
    ...
    if let Some(detected) = detected_type {
        ...
        tracing::warn!(
            "Could not detect magic bytes for file with extension '{}'",
            extension
        );
        return Err(AppError::InvalidFileType);
    }
...
fn is_text_file(extension: &str) -> bool {
    matches!(
        extension,
        "txt" | "csv" | "svg" | "json" | "xml" | "html" | "css" | "js"
    )
}
