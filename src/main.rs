    ...
    quic_server
        .run(move |connection| {
            ...
            async move {
                ...
                match server
                    .authenticate_and_register_connection(connection.clone())
                    .await
                {
...
async fn handle_quic_connection(
                ...
                match recv_stream.read_to_end(1024 * 1024).await {
                    Ok(data) => {
                        ...
                        match message_router
                            .route_message(&data, connection_id, user_id, &user_name)
                            .await
                        {
                            Ok(Some(response)) => {
    ...
    tracing::info!(
        "Finished handling QUIC connection: connection_id={}",
        connection_id
    );
    Ok(())
}
