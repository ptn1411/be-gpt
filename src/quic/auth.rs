...
pub enum AuthResponse {
    ...
    Success { user_id: String, user_name: String },
...
impl QuicAuthenticator {
    ...
    pub async fn authenticate_connection(
        ...
        match AuthService::verify_token(&auth_request.token, &self.jwt_secret) {
            Ok(claims) => {
                ...
                let user_id =
                    Uuid::parse_str(&claims.sub).map_err(|_| QuicAuthError::InvalidUserId)?;
            ...
            Err(e) => {
                ...
                let (code, message) = match e {
                    crate::error::AppError::TokenExpired => ("TOKEN_EXPIRED", "Token has expired"),
    ...
    pub fn authenticate_token(&self, token: &str) -> Result<(Uuid, String), QuicAuthError> {
        ...
        // Parse user ID from claims
        let user_id = Uuid::parse_str(&claims.sub).map_err(|_| QuicAuthError::InvalidUserId)?;
