...
use crate::quic::connection_manager::{
    Connection as ManagedConnection, ConnectionId, ConnectionManager, QuicConnection,
};
...
impl QuicServer {
    ...
    pub async fn update_config(
        ...
        info!(
            "  Old config: enabled={}, port={}, max_connections={}, max_streams={}",
            self.config.enabled,
            self.config.port,
        ...
        info!(
            "  New config: enabled={}, port={}, max_connections={}, max_streams={}",
            new_config.enabled,
            new_config.port,
        ...
        if requires_reinit {
            ...
            if state == ServerState::Running || state == ServerState::Initialized {
                ...
                warn!(
                    "The server must be stopped and reinitialized for these changes to take effect"
                );
            }
        }
    ...
    pub async fn initialize(&mut self) -> Result<(), QuicServerError> {
        ...
        transport_config
            .max_concurrent_uni_streams(self.config.max_streams_per_connection.try_into().unwrap());
        ...
        info!("QUIC server bound to {}", self.bound_addr.as_ref().unwrap());
    ...
    pub async fn handle_connection(&self, connection: Connection) -> Result<(), QuicServerError> {
        ...
        info!("Connection from {} closed: {}", remote_addr, closed);
    ...
    pub async fn authenticate_and_register_connection(
        ...
        let authenticator = self
            .authenticator
            .as_ref()
            .ok_or_else(|| QuicServerError::Config("Authenticator not configured".to_string()))?;
        ...
        let (send_stream, recv_stream) = match connection.accept_bi().await {
            ...
            Err(e) => {
                error!(
                    "Failed to accept authentication stream from {}: {}",
                    remote_addr, e
                );
        ...
        let (user_id, user_name) = match authenticator
            .authenticate_connection(recv_stream, send_stream)
            .await
        {
        ...
        connection_manager
            ...
            .await
            .map_err(|e| {
                QuicServerError::Config(format!("Failed to register connection: {}", e))
            })?;
    ...
    pub async fn receive_from_stream(
        ...
        match recv_stream.read(buffer).await {
            ...
            Err(e) => Err(QuicServerError::Endpoint(format!(
    ...
    pub async fn read_stream_to_end(
        ...
        let data = recv_stream.read_to_end(max_size).await.map_err(|e| {
            QuicServerError::Endpoint(format!("Failed to read stream to end: {}", e))
        })?;
    ...
    pub async fn handle_stream_with_allocator(
        ...
        let stream_allocator = self.stream_allocator.as_ref().ok_or_else(|| {
            QuicServerError::Config("Stream allocator not configured".to_string())
        })?;
    ...
    pub async fn handle_migration_failure(
        ...
        error!(
            "Handling migration failure for connection {}",
            connection_id
        );
        ...
        connection_manager
            ...
            .await
            .map_err(|e| {
                QuicServerError::Config(format!("Failed to mark migration as failed: {}", e))
            })?;
