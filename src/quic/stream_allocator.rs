...
impl MessageType {
    ...
    pub fn stream_range(&self) -> StreamRange {
        match self {
            ...
            MessageType::FileTransfer => StreamRange {
                start: 100,
                end: 199,
            },
            MessageType::BotCommand => StreamRange {
                start: 200,
                end: 299,
            },
        }
...
impl StreamAllocator {
    ...
    pub async fn get_active_streams(
        &self,
        connection_id: ConnectionId,
    ) -> Result<Vec<u64>, StreamAllocatorError> {
        let connections = self.connections.read().await;
        let conn_streams = connections
            .get(&connection_id)
            .ok_or(StreamAllocatorError::ConnectionNotFound(connection_id))?;
    ...
    pub async fn get_stats(&self) -> StreamAllocatorStats {
        ...
        StreamAllocatorStats {
            ...
            file_streams: *streams_by_type
                .get(&MessageType::FileTransfer)
                .unwrap_or(&0),
...
mod tests {
    ...
    fn test_message_type_stream_range() {
        assert_eq!(
            MessageType::Control.stream_range(),
            StreamRange { start: 0, end: 0 }
        );
        assert_eq!(
            MessageType::ChatMessage.stream_range(),
            StreamRange { start: 1, end: 99 }
        );
        assert_eq!(
            MessageType::FileTransfer.stream_range(),
            StreamRange {
                start: 100,
                end: 199
            }
        ...
        assert_eq!(
            MessageType::BotCommand.stream_range(),
            StreamRange {
                start: 200,
                end: 299
            }
    ...
    fn test_message_type_from_stream_id() {
        assert_eq!(
            MessageType::from_stream_id(0).unwrap(),
            MessageType::Control
        );
        assert_eq!(
            MessageType::from_stream_id(1).unwrap(),
            MessageType::ChatMessage
        );
        assert_eq!(
            MessageType::from_stream_id(50).unwrap(),
            MessageType::ChatMessage
        );
        assert_eq!(
            MessageType::from_stream_id(99).unwrap(),
            MessageType::ChatMessage
        );
        assert_eq!(
            MessageType::from_stream_id(100).unwrap(),
            MessageType::FileTransfer
        );
        assert_eq!(
            MessageType::from_stream_id(150).unwrap(),
            MessageType::FileTransfer
        );
        assert_eq!(
            MessageType::from_stream_id(199).unwrap(),
            MessageType::FileTransfer
        );
        assert_eq!(
            MessageType::from_stream_id(200).unwrap(),
            MessageType::BotCommand
        );
        assert_eq!(
            MessageType::from_stream_id(250).unwrap(),
            MessageType::BotCommand
        );
        assert_eq!(
            MessageType::from_stream_id(299).unwrap(),
            MessageType::BotCommand
        );
    ...
    async fn test_allocate_stream() {
        ...
        let stream_id = allocator
            .allocate_stream(conn_id, MessageType::Control)
            .await
            .unwrap();
        ...
        let stream_id = allocator
            .allocate_stream(conn_id, MessageType::ChatMessage)
            .await
            .unwrap();
    ...
    async fn test_allocate_multiple_message_types() {
        ...
        let control_stream = allocator
            .allocate_stream(conn_id, MessageType::Control)
            .await
            .unwrap();
        let chat_stream = allocator
            .allocate_stream(conn_id, MessageType::ChatMessage)
            .await
            .unwrap();
        let file_stream = allocator
            .allocate_stream(conn_id, MessageType::FileTransfer)
            .await
            .unwrap();
        let bot_stream = allocator
            .allocate_stream(conn_id, MessageType::BotCommand)
            .await
            .unwrap();
    ...
    async fn test_active_stream_count_for_type() {
        ...
        let chat_count = allocator
            .active_stream_count_for_type(conn_id, MessageType::ChatMessage)
            .await
            .unwrap();
        let file_count = allocator
            .active_stream_count_for_type(conn_id, MessageType::FileTransfer)
            .await
            .unwrap();
    ...
    async fn test_get_active_streams() {
        ...
        let stream1 = allocator
            .allocate_stream(conn_id, MessageType::ChatMessage)
            .await
            .unwrap();
        let stream2 = allocator
            .allocate_stream(conn_id, MessageType::FileTransfer)
            .await
            .unwrap();
